...
//			require Kohana::find_file('vendor', 'swift/swift_required');
	...
	public static function instance($cname = NULL)
	{
		...
		{
			isset ( self::$config[ $cname ] ) OR self::$config = Kohana::$config->load('email')->get(Kohana::$environment);
			new self( $cname, self::$config[ $cname ] );
		}
	...
	public static function send($cname, $subject, $body, $from, $to, $mime_type = 'text/plain', $charset = 'utf-8')
	{
		...
		$message->setTo( $to );
		if ( ! empty( self::$config[ $cname ][ 'from' ] ) )
			$message->setFrom( self::$config[ $cname ][ 'from' ] )->setReplyTo( $from );
		else
